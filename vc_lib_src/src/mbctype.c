    ...
    if (!(ptd->_ownlocale & __globallocalestatus) || !ptd->ptlocinfo) {
        ...
        __try {
            if ((ptmbci = ptd->ptmbcinfo) != __ptmbcinfo) {
                 ...
                 */
                if ((ptmbci != NULL) &&
                    (InterlockedDecrement((volatile LONG*) & (ptmbci->refcount)) == 0) &&
        ...
        if (ptmbci != NULL) {
            *ptmbci = *(ptd->ptmbcinfo);
            ...
            if ((retcode = _setmbcp_nolock(codepage, ptmbci)) == 0) {
                if (InterlockedDecrement((volatile LONG*)&ptd->ptmbcinfo->refcount) == 0 &&
                ...
                if (!(ptd->_ownlocale & _PER_THREAD_LOCALE_BIT) &&
                    ...
                    __try {
                        ...
                        if (InterlockedDecrement((volatile LONG*)&__ptmbcinfo->refcount) == 0 &&
