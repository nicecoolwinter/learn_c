...
)
{
    ...
    typedef BOOL (APIENTRY * PFNGetUserObjectInformation)(HANDLE, int, PVOID, DWORD,
            LPDWORD);
    void* pfn = NULL;
...
#endif  /* _UNICODE */
            pfnGetUserObjectInformation = (PFNGetUserObjectInformation) _encode_pointer(
                                              pfn);
    ...
    if (pfnGetProcessWindowStation != enull &&
        ...
        if (NULL == (hwinsta = ((PFNGetProcessWindowStation) _decode_pointer(
    ...
    if (fNonInteractive) {
        ...
        if (hWndParent != NULL && pfnGetLastActivePopup != enull) {
            hWndParent = ((PFNGetLastActivePopup) _decode_pointer(pfnGetLastActivePopup))(
                             hWndParent);
        }
    }

    return ((PFNMessageBox) _decode_pointer(pfnMessageBox))(hWndParent, lpText,
            lpCaption, uType);
}
