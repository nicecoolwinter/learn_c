...
)
{
    ...
    if (pwcs) {
        ...
        if (_loc_update.GetLocaleT()->locinfo->lc_handle[LC_CTYPE] == _CLOCALEHANDLE) {
            return strlen(s);
        } else if ((count = MultiByteToWideChar(
...
)
{
    ...
    errno_t retvalue = 0;
    /* validation section */
    _VALIDATE_RETURN_ERRCODE((pwcs == NULL && sizeInWords == 0) || (pwcs != NULL &&
    ...
    _LocaleUpdate _loc_update(plocinfo);
    /* Call a non-deprecated helper to do the work. */
    retsize = _mbstowcs_l_helper(pwcs, s, (n > sizeInWords ? sizeInWords : n),
