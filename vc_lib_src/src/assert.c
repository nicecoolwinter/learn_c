...
#endif  /* _UNICODE */
{
    ...
    if (osplatform == VER_PLATFORM_WIN32_WINDOWS) {
        ...
        if (expr && *expr) {
            errno_t e = _ERRCHECK_EINVAL_ERANGE(wcstombs_s(NULL, aexpr, ASSERTBUFSZ, expr,
                                                _TRUNCATE));
            ...
            if (e == STRUNCATE) {
                ...
                _ERRCHECK(strcpy_s(aexpr, ASSERTBUFSZ,
        ...
        if (filename && *filename) {
            errno_t e = _ERRCHECK_EINVAL_ERANGE(wcstombs_s(NULL, afilename, ASSERTBUFSZ,
                                                filename, _TRUNCATE));
    ...
    if ((_set_error_mode(_REPORT_ERRMODE) == _OUT_TO_STDERR) ||
...
#ifdef _UNICODE
        {
            ...
            {
                if (swprintf(assertbuf, ASSERTBUFSZ, _assertstring, expr, filename,
                lineno) >= 0) {
                    if (GetFileType(hErr) == FILE_TYPE_CHAR) {
                        if (WriteConsoleW(hErr, assertbuf, (unsigned long)wcslen(assertbuf), &written,
                        NULL)) {
                            abort();
                        }
                    }
        ...
        if ((sizeof(PROGINTRO) / sizeof(TCHAR)) + _tcslen(progname) + NEWLINESZ >
            MAXLINELEN) {
            pch += ((sizeof(PROGINTRO) / sizeof(TCHAR)) + _tcslen(progname) + NEWLINESZ) -
                   MAXLINELEN;
            _ERRCHECK(_tcsncpy_s(pch, (MAX_PATH + 1) - (pch - progname), dotdotdot,
                                 DOTDOTDOTSZ));
        }
        ...
        if ((sizeof(FILEINTRO) / sizeof(TCHAR)) + _tcslen(filename) + NEWLINESZ >
            MAXLINELEN) {
            size_t p, len, ffn;
            pch = (TCHAR*) filename;
            ...
            if ((ffn - ffn / 3) < (len - p) && ffn / 3 > p) {
                ...
                _ERRCHECK(_tcscat_s(assertbuf, ASSERTBUFSZ, dotdotdot));
                _ERRCHECK(_tcscat_s(assertbuf, ASSERTBUFSZ,
                                    pch + len - (ffn / 3 - ffn / 6 - 2)));
            }
        } else
        ...
        _ERRCHECK(_itot_s(lineno, assertbuf + _tcslen(assertbuf),
                          ASSERTBUFSZ - _tcslen(assertbuf), 10));
        _ERRCHECK(_tcscat_s(assertbuf, ASSERTBUFSZ, dblnewline));
        /*
         * Line 5: message line
         */
