...
)
{
    ...
    if ((ix >= 0) && (*env != NULL)) {
        ...
        if (!remove)  {
            ...
            if ((ix + 2) < ix ||
                ...
                (env = (_TSCHAR**)_recalloc_crt(_tenviron, sizeof(_TSCHAR*),
                                                (ix + 2))) == NULL) {
                return -1;
            }
     ...
     */
    if (primary &&
        (name = (_TSCHAR*)_calloc_crt((_tcslen(option) + 2),
                                      sizeof(_TSCHAR))) != NULL) {
        ...
        *value++ = _T('\0');
        ...
        if (f_use == USE_A) {
            ...
            if ((size = WideCharToMultiByte(CP_ACP, 0, name, -1, NULL, 0, NULL,
                                            NULL)) == 0) {
                retval = -1;
            }
            ...
            /* convert it */
            else if (WideCharToMultiByte(CP_ACP, 0, name, -1, c_name, size, NULL,
                                         NULL) == 0) {
                retval = -1;
            } else if (!remove) {
                if ((size = WideCharToMultiByte(CP_ACP, 0, value, -1, NULL, 0, NULL,
                                                NULL)) == 0) {
                    retval = -1;
                }
                /* allocate space for variable */
                ...
                /* convert it */
                else if (WideCharToMultiByte(CP_ACP, 0, value, -1, c_value, size, NULL,
                                             NULL) == 0) {
                    retval = -1;
                }
            }
