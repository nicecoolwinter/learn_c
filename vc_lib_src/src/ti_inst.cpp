...
const char* type_info::name(__type_info_node* __ptype_info_node)
const   //17.3.4.2.5
{
...
void type_info::_Type_info_dtor(type_info* _This)
{
    ...
    if (_This->_m_data != NULL) {
         ...
         */
        for (__type_info_node* pNode = __type_info_root_node.next,
             *tmpNode = &__type_info_root_node;
             pNode != NULL;
             pNode = tmpNode) {
...
__RELIABILITY_CONTRACT
const char* type_info::_Name_base(const type_info* _This,
                                  __type_info_node* __ptype_info_node)
{
    ...
    if (_This->_m_data == NULL) {
         ...
         */
        for (len = strlen((char*)pTmpUndName); len-- > 0 &&
             ((char*)pTmpUndName)[len] == ' ';) {
        ...
        if (_This->_m_data == NULL) {
             ...
             */
            __type_info_node* pNode = (__type_info_node*)_malloc_base(sizeof(
                                          __type_info_node));
            ...
            if (pNode != NULL) {
                ...
                if ((((type_info*)_This)->_m_data = _malloc_base(len + 2)) != NULL) {
                    _ERRCHECK(strcpy_s((char*)((type_info*)_This)->_m_data, len + 2,
                                       (char*)pTmpUndName));
