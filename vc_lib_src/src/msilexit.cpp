    ...
    if (__global_lock() == true) {
        try {
            ...
            if ((*__pexit_list_size) - 1 <
                ((size_t)((char*)(*__ponexitend) - (char*)(*__ponexitbegin))) / sizeof(
                    _CPVFV)) {
                /*
...
extern "C" void __clrcall _exit_callback(void)
{
    ...
    if ((__onexitbegin != (_CPVFV*) - 1) && (__onexitbegin != NULL) &&
        (__onexitend != NULL)) {
...
extern "C" int __clrcall _initatexit_app_domain()
{
    if (__alloc_global_lock() == true) {
        ...
        __onexitend_app_domain = __onexitbegin_app_domain = (_CPVFV*)
                                 tmp_int_ptr.ToPointer();
...
extern "C" void __clrcall _app_exit_callback(void)
{
     ...
     * case then we will need to have lock around this function.
     */
    if ((__onexitbegin_app_domain != (_CPVFV*) - 1) &&
        (__onexitbegin_app_domain != NULL) && (__onexitend_app_domain != NULL)) {
        __try {
...
{
    return _atexit_helper(func, &__exit_list_size_app_domain,
