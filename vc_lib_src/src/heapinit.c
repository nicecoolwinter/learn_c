...
int __cdecl __heap_select(void)
{
    ...
    if (HeapStringSize = GetEnvironmentVariableA(__HEAP_ENV_STRING,
                         env_heap_select_string, 0)) {
        env_heap_select_string = HeapAlloc(GetProcessHeap(), 0, HeapStringSize);

        if (env_heap_select_string) {
            actual_size = GetEnvironmentVariableA(__HEAP_ENV_STRING, env_heap_select_string,
                                                  HeapStringSize);
            ...
            if (actual_size > 0 && actual_size <= HeapStringSize) {
                ...
                if (!strncmp(__GLOBAL_HEAP_SELECTOR, env_heap_select_string,
                             sizeof(__GLOBAL_HEAP_SELECTOR) - 1)) {
                    env_heap_type = env_heap_select_string;
                } else {
                    char* env_app_name = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
                                                   MAX_PATH + 1);
