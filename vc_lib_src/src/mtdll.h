...
struct _tiddata {
    ...
    void*
    _curexcspec;    /* for handling exceptions thrown from std::unexpected */
    ...
    unsigned long
    __initDomain;     /* initial domain used by _beginthread[ex] for managed function */
};
...
_ptiddata __cdecl _getptd_noexit(
    void);  /* return address of per-thread CRT data - doesn't exit on malloc failure */
_CRTIMP void WINAPI _freefls(__inout_opt void*
                             _PerFiberData);          /* free up per-fiber CRT data block */
void __cdecl _freeptd(__inout_opt _ptiddata
                      _Ptd); /* free up a per-thread CRT data block */
_CRTIMP void __cdecl _initptd(__inout _ptiddata _Ptd,
                              __in_opt pthreadlocinfo _Locale); /* initialize a per-thread CRT data block */
