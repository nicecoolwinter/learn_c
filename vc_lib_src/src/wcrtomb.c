...
)
{
     ...
     */
    _VALIDATE_RETURN_ERRCODE((dst == NULL && sizeInBytes == 0) ||
                             (dst != NULL), EINVAL);
...
)
{
    ...
    /* validation section */
    _VALIDATE_RETURN_ERRCODE((dst == NULL && sizeInBytes == 0) || (dst != NULL &&
                             sizeInBytes > 0), EINVAL);
    ...
    /* Call a non-deprecated helper to do the work. */
    retsize = _wcsrtombs_helper(dst, src, (n > sizeInBytes ? sizeInBytes : n),
                                pmbst);
