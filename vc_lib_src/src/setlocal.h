...
#ifndef _M_CEE_PURE
_CRTIMP extern LCID
__lc_handle[];       /* locale "handles" - ignores country info */
...
_CRTIMP extern int
__lc_clike;          /* if first 127 characters of current locale
...
BOOL __cdecl __get_qualified_locale(__in_opt const LPLC_STRINGS _LpInStr,
                                    __out_opt LPLC_ID _LpOutId, __out_opt LPLC_STRINGS _LpOutStr);

int __cdecl __getlocaleinfo(__in_opt _locale_t _Locale, __in int _Lc_type,
                            LCID _LocaleId, LCTYPE _FieldType, __out void* _Address);
