...
#ifdef _UNICODE
int __cdecl _swinput_s(const wchar_t* _String, size_t _Count,
                       const wchar_t* _Format, va_list _ArgList)
#else  /* _UNICODE */
int __cdecl _sinput_s(const char* _String, size_t _Count, const char* _Format,
                      va_list _ArgList)
#endif  /* _UNICODE */
{
    FILE stream;
                                ...
                                int* pmalloc_FloatStrFlag)
{
    ...
    if (nFloatStrUsed == (*pnFloatStrSz)) {
        if ((*pFloatStr) == floatstring) {
            if (((*pFloatStr) = (_TCHAR*)_calloc_crt((*pnFloatStrSz),
                                2 * sizeof(_TCHAR))) == NULL) {
                return FALSE;
            }
            ...
            if ((tmpPointer = (_TCHAR*)_recalloc_crt((*pFloatStr), (*pnFloatStrSz),
                              2 * sizeof(_TCHAR))) == NULL) {
                return FALSE;
            }
...
#ifndef _SECURE_SCANF
    return input(reinterpret_cast<const _TUCHAR*>(format),
                 arglist);   /* get the input */
#else  /* _SECURE_SCANF */
    return input_s(reinterpret_cast<const _TUCHAR*>(format),
                   arglist);   /* get the input */
#endif  /* _SECURE_SCANF */
}
...
int __cdecl _CPRSCANF(const _TCHAR* format, ...)
{
    va_list arglist;
    va_start(arglist, format);
    return _CPRINPUT_L(reinterpret_cast<const _TUCHAR*>(format), NULL,
                       arglist);   /* get the input */
}
...
int __cdecl _CPRSCANF_L(const _TCHAR* format, _locale_t plocinfo, ...)
{
    ...
    return _CPRINPUT_L(reinterpret_cast<const _TUCHAR*>(format), plocinfo,
                       arglist);   /* get the input */
}
...
#ifdef CPRFLAG
#ifndef _SECURE_SCANF
static int __cdecl _cprinput_l(const _TUCHAR* format, _locale_t plocinfo,
                               va_list arglist)
#else  /* _SECURE_SCANF */
static int __cdecl _cprinput_s_l(const _TUCHAR* format, _locale_t plocinfo,
                                 va_list arglist)
#endif  /* _SECURE_SCANF */
#else  /* CPRFLAG */
#ifndef _SECURE_SCANF
int __cdecl _tinput_l(FILE* stream, const _TUCHAR* format, _locale_t plocinfo,
                      va_list arglist)
#else  /* _SECURE_SCANF */
int __cdecl _tinput_s_l(FILE* stream, const _TUCHAR* format, _locale_t plocinfo,
                        va_list arglist)
#endif  /* _SECURE_SCANF */
#endif  /* CPRFLAG */
#endif  /* _SAFECRT_IMPL */
...
DEFAULT_LABEL:
            ...
            if (!widthset || width) {
                ...
                if (!suppress && (comchr == _T('c') || comchr == _T('s') ||
                                  comchr == LEFT_BRACKET)) {
                    arglist = arglistsave;
                            ...
                            if (!suppress) {
...
#endif  /* _UNICODE */
                                {
                                    ...
                                    *(char*)pointer = (char)ch;
                                    ...
                                    if (_ERRCHECK_EINVAL_ERANGE(wctomb_s(&temp, (char*)pointer, MB_LEN_MAX,
                                                                         ch)) == 0) {
                    ...
                    if (started)
                        if (!suppress) {
...
#ifdef _UNICODE
                            {
                                ...
                                _ERRCHECK_EINVAL_ERANGE(wcstombs_s(NULL, cfloatstring, cfslength, pFloatStr,
                                                                   cfslength - 1));
...
{
    return _ISDIGIT(chr) ? chr : (chr & ~(_T('a') - _T('A'))) - _T('A') + 10 +
           _T('0');
}
