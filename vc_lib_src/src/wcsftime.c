...
extern "C" size_t __cdecl _Strftime_l(char* string, size_t maxsize,
                                      const char* format,
...
)
{
    ...
    if (_ERRCHECK_EINVAL_ERANGE(_wcstombs_s_l(NULL, format, flen * 2, wformat,
                                flen * 2 - 1, plocinfo)) != 0) {
        goto done;
    }

    if (_Strftime_l(string, maxsize * 2, format, timeptr, 0, plocinfo)) {
        if (_ERRCHECK_EINVAL_ERANGE(_mbstowcs_s_l(&retval, wstring, maxsize, string,
                                    _TRUNCATE, plocinfo)) != 0) {
