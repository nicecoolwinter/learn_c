...
)
{
    ...
    __try {
        ...
        if ((_ERRCHECK_EINVAL(_tdupenv_s_crt(&envbuf, NULL, _T("COMSPEC"))) != 0) ||
            (envbuf == NULL)) {
        ...
        CommandLineSize = _tcslen(cmdexe) + _tcslen(_T(" /c ")) + (_tcslen(
                              cmdstring)) + 1;
...
)
{
    ...
    if ((pstream != NULL) ||
        ...
        ((newptr = (IDpair*)_recalloc_crt((void*)__idpairs, (__idtabsiz + 1),
                                          sizeof(IDpair))) == NULL))
