...
)
{
    if (lc_type == LC_STR_TYPE) {
        ...
        if ((outsize = __crtGetLocaleInfoA(plocinfo, localehandle, fieldtype, pcbuffer,
                                           buffersize, 0))
            == 0) {
            ...
            if ((buffersize = __crtGetLocaleInfoA(plocinfo, localehandle, fieldtype, NULL,
                                                  0, 0))
                == 0) {
                goto error;
            }
...
error:
        ...
        if (__crtGetLocaleInfoW(plocinfo, localehandle, fieldtype, (LPWSTR)&wcbuffer,
                                buffersize, 0) == 0) {
            return -1;
        }
        ...
        *(char*)charaddress = 0;
        ...
        for (i = 0; i < INT_CHAR_CNT; i++) {
            if (isdigit(((unsigned char)c = (unsigned char)wcbuffer[i]))) {
                *(unsigned char*)charaddress = (unsigned char)(10 * (int)(*charaddress) +
                                               (c - '0'));
            } else {
                break;
            }
