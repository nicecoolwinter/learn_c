...
)
{
    ...
    if (osplatform == VER_PLATFORM_WIN32_WINDOWS) {
        char szaFile[DBGRPT_MAX_MSG], szaModule[DBGRPT_MAX_MSG],
             szaFormat[DBGRPT_MAX_MSG];
        errno_t e = 0;

        if (szFile && *szFile) {
            e = _ERRCHECK_EINVAL_ERANGE(wcstombs_s(NULL, szaFile, DBGRPT_MAX_MSG, szFile,
                                                   _TRUNCATE));
            ...
            if (e == STRUNCATE) {
                ...
                _ERRCHECK(strcpy_s(szaFile, DBGRPT_MAX_MSG,
        ...
        if (szModule && *szModule) {
            e = _ERRCHECK_EINVAL_ERANGE(wcstombs_s(NULL, szaModule, DBGRPT_MAX_MSG,
                                                   szModule, _TRUNCATE));
            ...
            if (e == STRUNCATE) {
                ...
                _ERRCHECK(strcpy_s(szaModule, DBGRPT_MAX_MSG,
                                   "Expression cannot be displayed on Win9x"));
            }
        } else {
            szaModule[0] = '\0';
        ...
        if (szFormat && *szFormat) {
            e = _ERRCHECK_EINVAL_ERANGE(wcstombs_s(NULL, szaFormat, DBGRPT_MAX_MSG,
                                                   szFormat, _TRUNCATE));
...
)
{
    ...
    if (_tcslen(szShortProgName) > MAXLINELEN) {
        ...
        _ERRCHECK(_tcsncpy_s(szShortProgName, MAX_PATH - (szShortProgName - szExeName),
                             dotdotdot, DOTDOTDOTSZ));
    }
    ...
    _ERRCHECK_SPRINTF(szlen = _sntprintf_s(szOutMessage, DBGRPT_MAX_MSG,
                                           DBGRPT_MAX_MSG - 1,
