...
#define _putwc_nolock(_c, _stream)   ((_stream->_cnt -= sizeof(wchar_t)) >= 0 ? (wint_t) (0xffff & (*((wchar_t *)(_stream->_ptr))++ = (wchar_t)_c)) : WEOF)
     ...
     */
#ifndef _UNICODE
    int __cdecl _soutput_s(char* _Dst, size_t _Size, const char* _Format,
                           va_list _ArgList)
#else  /* _UNICODE */
    int __cdecl _swoutput_s(wchar_t* _Dst, size_t _Size, const wchar_t* _Format,
                            va_list _ArgList)
#endif  /* _UNICODE */
    {
        FILE stream;
...
#ifndef CPRFLAG
    int _tvalidate_param_reuse(struct positional_param* pos,
                               enum ARG_TYPE get_fn_type, TCHAR cur_type, int cur_flags)
    {
        int stored_value = 0 ;
        int new_value = 0 ;
        ...
        if (stored_value || new_value) {
            ...
            */
            return ((stored_value == new_value) &&
                    (_IS_FMTTYPE_WIDE(pos->flags) == _IS_FMTTYPE_WIDE(cur_flags)));
        }
                ...
                if ((state == ST_PERCENT) && (*format != _T('%'))) {
                    ...
                    if (format_type == FMT_TYPE_POSITIONAL) {
                        ...
                        if (pass == FORMAT_POSSCAN_PASS) {
                            ...
                            _VALIDATE_RETURN(((type_pos >= 0) && (*end_pos == POSITION_CHAR) &&
                                              (type_pos < _ARGMAX)), EINVAL, -1);
                ...
                case ST_WIDTH:
                    ...
                    if (ch == _T('*')) {
                        ...
                        if (format_type == FMT_TYPE_NONPOSITIONAL) {
                            ...
                            if (pass == FORMAT_POSSCAN_PASS) {
                                _VALIDATE_RETURN(((width_pos >= 0) && (*end_pos == POSITION_CHAR) &&
                                                  (type_pos < _ARGMAX)), EINVAL, -1);
                                /* Update max_pos with the current maximum pos argument */
                ...
                case ST_PRECIS:
                    ...
                    if (ch == _T('*')) {
                        ...
                        if (format_type == FMT_TYPE_NONPOSITIONAL) {
                            ...
                            if (pass == FORMAT_POSSCAN_PASS) {
                                _VALIDATE_RETURN(((precis_pos >= 0) && (*end_pos == POSITION_CHAR) &&
                                                  (type_pos < _ARGMAX)), EINVAL, -1);
                                /* Update max_pos with the current maximum pos argument */
...
#ifndef _SAFECRT_IMPL
                                _cfltcvt_l(&tmp.x, text.sz, buffersize, (char)ch, precision, capexp,
...
COMMON_INT: {
...
#ifdef POSITIONAL_PARAMETERS
                                            {
                                                if (format_type == FMT_TYPE_NONPOSITIONAL) {
                                                    ...
                                                    if (pass == FORMAT_POSSCAN_PASS) {
                                                        ...
                                                        GET_ARG(get_int_arg, pos_value[type_pos].arg_ptr, l,
                            ...
                            textlen = (int)((char*)&buffer.sz[BUFFERSIZE - 1] -
                                            text.sz); /* compute length of number */
