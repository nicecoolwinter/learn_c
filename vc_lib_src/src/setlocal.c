...
)
{
    ...
    for (category = LC_MIN; category <= LC_MAX; ++category) {
        ...
        _ASSERTE(((ptloci->lc_category[category].wlocale != NULL) &&
                  (ptloci->lc_category[category].wrefcount != NULL)) ||
                 ((ptloci->lc_category[category].wlocale == NULL) &&
                  (ptloci->lc_category[category].wrefcount == NULL)));
...
int __cdecl _configthreadlocale(int i)
{
     ...
     */
    _ptiddata ptd = _getptd();
    int retval = (ptd->_ownlocale & _PER_THREAD_LOCALE_BIT) == 0 ?
                 _DISABLE_PER_THREAD_LOCALE : _ENABLE_PER_THREAD_LOCALE;
...
)
{
    ...
    __try {
        if ((ptloci = _calloc_crt(sizeof(threadlocinfo), 1)) != NULL) {
            ...
            if ((ptloci != NULL) &&
                (retval = _setlocale_nolock(ptloci, _category, _locale))) {
                /*
                ...
                */
                ...
                __try {
                    ...
                    if (!(ptd->_ownlocale & _PER_THREAD_LOCALE_BIT) &&
                        ...
                        memcpy(__lc_handle, __ptlocinfo->lc_handle,
                               sizeof(LCID) * (LC_MAX - LC_MIN + 1));
    ...
    if (_category != LC_ALL) {
        ...
        if (_locale != NULL) {
            if ((_locale[0] == 'L') && (_locale[1] == 'C') && (_locale[2] == '_')) {
                ...
                if (retval = _expandlocale((char*)_locale, lctemp, _countof(lctemp), NULL, NULL,
                                           _category)) {
...
)
{
    ...
    struct _is_ctype_compatible* _Lcid_c =
            _ptd->_setloc_data._Lcid_c; // __setloc_data._Lcid_c is array
    int _LOC_CCACHE = sizeof(_ptd->_setloc_data._Lcid_c) / sizeof(
                          struct _is_ctype_compatible);
    struct _is_ctype_compatible buf1, buf2;

    if (!_expandlocale((char*)locale, lctemp, _countof(lctemp), &idtemp, &cptemp,
                       category)) {
        ...
        if (i == _LOC_CCACHE) {
            if (__crtGetStringTypeA(NULL, CT_CTYPE1,
                ...
                for (j = 0; j < sizeof(_first_127char); j++)
                    out[j] = out[j] &
                             (_UPPER | _LOWER | _DIGIT | _SPACE | _PUNCT | _CONTROL | _BLANK | _HEX |
                              _ALPHA);

                if (!memcmp(out, _ctype_loc_style,
                            (sizeof(_first_127char) / sizeof(char))*sizeof(short))) {
...
static char* __cdecl _setlocale_get_all(pthreadlocinfo ploci)
{
    ...
    refcountSize = sizeof(int) + (MAX_LC_LEN + 1) * (LC_MAX - LC_MIN + 1) +
                   CATNAMES_LEN;
    ...
    *refcount = 1;
    ...
    for (i = LC_MIN + 1; ; i++) {
        _strcats(pch, cch, 3, __lc_category[i].catname, "=",
                 ploci->lc_category[i].locale);
...
)
{
    ...
    if (charactersInExpression >= MAX_LC_LEN ||
    ...
    if (id) {
        memcpy((void*)id, (void*)pcacheid,
               sizeof(*pcacheid));    /* possibly return LC_ID */
    }

    if (cp) {
        memcpy((void*)cp, (void*)pcachecp,
               sizeof(*pcachecp));    /* possibly return cp */
    }
...
int __cdecl __init_dummy(pthreadlocinfo
                         ploci)   /* default routine for locale initializer */
{
    return 0;
}
...
int __lc_strtolc(LC_STRINGS* names, const char* locale)
{
    ...
    if (locale[0] == '.' && locale[1] != '\0') {
        _ERRCHECK(strncpy_s(names->szCodePage, _countof(names->szCodePage), &locale[1],
                            MAX_CP_LEN - 1));
    ...
    for (i = 0; ; i++) {
        ...
        if ((i == 0) && (len < MAX_LANG_LEN) && (ch != '.')) {
            _ERRCHECK(strncpy_s(names->szLanguage, _countof(names->szLanguage), locale,
                                len));
            ...
            _ERRCHECK(strncpy_s(names->szCodePage, _countof(names->szCodePage), locale,
                                len));
        } else {
