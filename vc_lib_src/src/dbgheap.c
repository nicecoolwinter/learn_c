...
extern "C" _CRTIMP long _crtBreakAlloc =
    -1L;  /* Break on allocation by request number */
...
static unsigned char _bNoMansLandFill =
    0xFD;   /* fill no-man's land with this */
static unsigned char _bAlignLandFill  =
    0xED;   /* fill no-man's land for aligned routines */
static unsigned char _bDeadLandFill   =
    0xDD;   /* fill free objects with this */
...
)
{
    ...
    __try {
        ...
        /* forced failure */
        if ((_pfnAllocHook) &&
            !(*_pfnAllocHook)(_HOOK_ALLOC, NULL, nSize, nBlockUse, lRequest,
            ...
            if (nSize > (size_t)(_HEAP_MAXREQ - nNoMansLandSize - sizeof(
                                     _CrtMemBlockHeader))) {
...
)
{
    ...
    /* forced failure */
    if ((_pfnAllocHook) &&
        !(*_pfnAllocHook)(_HOOK_REALLOC, pUserData, nNewSize, nBlockUse, lRequest,
                          (const unsigned char*)szFileName, nLine)) {
        if (szFileName)
    ...
    if (nNewSize > (size_t)(_HEAP_MAXREQ - nNoMansLandSize - sizeof(
                                _CrtMemBlockHeader))) {
        _RPT1(_CRT_ERROR, "Invalid allocation size: %Iu bytes.\n", nNewSize);
        errno = ENOMEM;
        return NULL;
    ...
    if (nBlockUse != _NORMAL_BLOCK
        ...
        if (CheckBytes((unsigned char*)((uintptr_t)pUserData & ~(sizeof(
                                            uintptr_t) - 1)) - nAlignGapSize, _bAlignLandFill, nAlignGapSize)) {
            _RPT1(_CRT_ERROR,
    ...
    if (fIgnore) {
        ...
        if (_BLOCK_TYPE(pOldBlock->nBlockUse) == _CRT_BLOCK &&
            _BLOCK_TYPE(nBlockUse) == _NORMAL_BLOCK) {
...
)
{
    ...
    /* forced failure */
    if ((_pfnAllocHook) &&
        !(*_pfnAllocHook)(_HOOK_FREE, pUserData, 0, nBlockUse, 0L, NULL, 0)) {
...
)
{
    ...
    __try {
        ...
        for (pHead = _pFirstBlock; pHead != NULL; pHead = pHead->pBlockHeaderNext) {
            if (_BLOCK_TYPE(pHead->nBlockUse) >= 0 &&
                _BLOCK_TYPE(pHead->nBlockUse) < _MAX_BLOCKS) {
...
)
{
    ...
    for (i = 0; i < min((int)pHead->nDataSize, MAXPRINT); i++) {
        ...
        _ERRCHECK_SPRINTF(sprintf_s((char*)&valbuff[i * 3],
                                    _countof(valbuff) - (i * 3) , "%.2X ", ch));
    }
...
#pragma warning(disable: 4996)
                    ...
                    if (!_CrtIsValidPointer(pHead->szFileName, 1, FALSE) ||
                        IsBadReadPtr(pHead->szFileName, 1))
#pragma warning(pop)
                    {
...
)
{
    ...
    if ((ptr = (uintptr_t) _malloc_dbg(t_ptr + size + align + sizeof(
                                           _AlignMemBlockHdr), _NORMAL_BLOCK, f_name, line_n)) == (uintptr_t)NULL) {
        return NULL;
    }

    r_ptr = ((ptr + align + t_ptr + sizeof(_AlignMemBlockHdr) + offset) & ~align) -
            offset;
...
{
    return _aligned_offset_recalloc_dbg(memblock, count, size, align, offset, NULL,
                                        0);
}
...
)
{
    ...
    s_pHdr = (_AlignMemBlockHdr*)((uintptr_t)memblock & ~(sizeof(
                                      uintptr_t) - 1)) - 1;

    if (CheckBytes((unsigned char*)memblock - nNoMansLandSize, _bNoMansLandFill,
                   nNoMansLandSize)) {
        _RPT1(_CRT_ERROR,
    ...
    _VALIDATE_RETURN(IS_2_POW_N(align), EINVAL, NULL);
    _VALIDATE_RETURN(offset == 0 || offset < size, EINVAL, NULL);
    mov_sz = _msize(s_pHdr->pHead) - ((uintptr_t)memblock -
                                      (uintptr_t)s_pHdr->pHead);
    align = (align > sizeof(uintptr_t) ? align : sizeof(uintptr_t)) - 1;
    t_ptr = (0 - offset) & (sizeof(uintptr_t) - 1);
...
)
{
    ...
    size_orig = size * count;
    return _aligned_offset_realloc_dbg(memblock, size_orig, align, offset, f_name,
                                       line_n);
}
