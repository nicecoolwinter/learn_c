...
_CRT_INSECURE_DEPRECATE_GLOBALS(_get_amblksiz) _CRTIMP unsigned int* __cdecl
__p__amblksiz(void);
...
_CRTIMP unsigned long __cdecl _set_malloc_crt_max_wait(__in unsigned long
        _NewValue);
...
_CRTIMP __checkReturn __bcount_opt(_NewSize) void*   __cdecl _expand(
    __in void* _Memory, __in size_t _NewSize);
...
__checkReturn __bcount(_Size) void*           __cdecl _alloca(
    __in size_t _Size);
...
_CRTIMP __checkReturn int     __cdecl _heapadd(__in void* _Memory,
        __in size_t _Size);
...
_CRTIMP _CRT_MANAGED_HEAP_DEPRECATE int     __cdecl _heapwalk(
    __inout _HEAPINFO* _EntryInfo);
...
__inline void* _MarkAllocaS(__out_opt __crt_typefix(unsigned int*) void* _Ptr,
