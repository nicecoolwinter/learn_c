...
#ifdef _UNICODE
    _ERRCHECK(mbstowcs_s(NULL, bldmsg + wcslen(bldmsg),
                         _ERRMSGLEN_ - wcslen(bldmsg), sysErrorMsg, _ERRMSGLEN_ - wcslen(bldmsg) - 2));
#else  /* _UNICODE */
    _ERRCHECK(strncat_s(bldmsg, _ERRMSGLEN_, sysErrorMsg,
                        _ERRMSGLEN_ - strlen(bldmsg) - 2));
#endif  /* _UNICODE */
    _ERRCHECK(_tcscat_s(bldmsg, _ERRMSGLEN_, _T("\n")));
    return bldmsg;
}
...
)
{
    ...
    return _tcserror_s(buffer + _tcslen(buffer), sizeInTChars - _tcslen(buffer),
                       errno);
}
