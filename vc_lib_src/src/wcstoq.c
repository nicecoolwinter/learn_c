...
)
{
    if (__locale_changed == 0) {
        return (__int64) wcstoxq(&__initiallocalestructinfo, nptr,
                                 (const wchar_t**)endptr, ibase, 0);
    } else {
...
{
    if (__locale_changed == 0) {
        return wcstoxq(&__initiallocalestructinfo, nptr, (const wchar_t**)endptr, ibase,
                       FL_UNSIGNED);
    } else {
