...
volatile __declspec(appdomain) _new_handler_m __nhmHeap =
    (_new_handler_m) _encoded_null();
...
_MRTIMP _PHNDLR_m __cdecl signal(int signum, _PHNDLR_m sigact)
{
    ...
    _PHNDLR_m __psignal_func_old = (_PHNDLR_m) _decode_pointer(
                                       __psignal_func[signum]);
    __psignal_func[signum] = (_PHNDLR_m) _encode_pointer(sigact);
    void (__cdecl * pnative_signal_func_old)(int) = signal(signum,
            (void (__cdecl*)(int))__signal_thunk);
