...
typedef BOOL (WINAPI* PFN_FINDAC)(DWORD dwFlags, const GUID* lpExtensionGuid,
                                  ULONG ulSectionId, LPCWSTR lpStringToFind,
                                  PACTCTX_SECTION_KEYED_DATA ReturnedData);
typedef UINT(WINAPI* PFN_GETSYSTEMWINDOWSDIRECTORY)(LPWSTR lpBuffer,
        UINT uSize);
...
static BOOL __cdecl _check_manifest(HMODULE hDllHandle)
{
    ...
    /* check condition (1) */
    {
        ...
        pfnFindActCtxSectionStringW = (PFN_FINDAC) GetProcAddress(hKernel32,
    ...
    /* check condition (4) */
    {
        ...
        if (!(*pfnFindActCtxSectionStringW)(0, NULL,
                                            ACTIVATION_CONTEXT_SECTION_DLL_REDIRECTION, _CRT_DLL_FILENAME, &askd)) {
    ...
    /* check condition (5) */
    {
        ...
        pfnGetSystemWindowsDirectory = (PFN_GETSYSTEMWINDOWSDIRECTORY) GetProcAddress(
                                           hKernel32, "GetSystemWindowsDirectoryW");
        ...
        /* add a trailing backslash if not there already */
        if (systemRoot[systemRootLen - 1] != L'\\') {
            if (wcsncat_s(systemRoot, _countof(systemRoot), L"\\" _WINSXS_FOLDER,
                          _TRUNCATE) != 0) {
                /* error or not enough space */
                return TRUE;
            }
        } else {
            if (wcsncat_s(systemRoot, _countof(systemRoot), _WINSXS_FOLDER,
                          _TRUNCATE) != 0) {
                /* error or not enough space */
                return TRUE;
            }
    ...
    /* check condition (6) */
    {
        ...
        if (wcsncat_s(moduleFilePath, _countof(moduleFilePath), _MANIFEST_FILENAME,
                      _TRUNCATE) != 0) {
            /* error or truncation */
            return TRUE;
        }
