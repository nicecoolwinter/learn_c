...
#pragma warning(pop)
    ...
    if (osplatform == VER_PLATFORM_WIN32_NT) {
        ...
        if (ModuleHandle != NULL) {
            SetThreadStackGuaranteePointer = (PVOID) GetProcAddress(ModuleHandle,
    ...
    //
    pBspBase = (LPBYTE)(((ULONG_PTR)(((PNT_TIB)NtCurrentTeb())->StackBase) -
    ...
    //
    pMinGuard = (LPBYTE)((__getReg(__REG_IA64_RsBSP) + PageSize) & ~(ULONG_PTR)(
                             PageSize - 1));
    //
    ...
    if (VirtualAlloc(pMinGuard, BspRegionSize, MEM_COMMIT,
                     PAGE_READWRITE) == NULL ||
