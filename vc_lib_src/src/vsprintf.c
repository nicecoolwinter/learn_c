...
)
{
    ...
    retvalue = _vsnprintf_helper(_output_s_l, string, sizeInBytes, format, plocinfo,
                                 ap);
...
)
{
    ...
    if (sizeInBytes > count) {
        save_errno = errno;
        retvalue = _vsnprintf_helper(_output_s_l, string, count + 1, format, plocinfo,
                                     ap);
...
{
    int retval = _vsnprintf_helper(_output_p_l, string, count, format, plocinfo,
                                   ap);
    return (retval < 0 ? -1 : retval);
}
