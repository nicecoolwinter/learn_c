...
)
{
    ...
    retvalue = _vswprintf_helper(_woutput_s_l, string, sizeInWords, format,
                                 plocinfo, ap);
...
)
{
    ...
    if (sizeInWords > count) {
        save_errno = errno;
        retvalue = _vswprintf_helper(_woutput_s_l, string, count + 1, format, plocinfo,
                                     ap);
...
{
    int retval = _vswprintf_helper(_woutput_p_l, string, count, format, plocinfo,
                                   ap);
    return (retval < 0 ? -1 : retval);
}
