...
struct _Fac_node {
    ...
    __CLR_OR_THIS_CALL _Fac_node(_Fac_node* _Nextarg,
                                 std::locale::facet* _Facptrarg)
...
__PURE_APPDOMAIN_GLOBAL static std::locale::_Locimp* global_locale =
    0; // pointer to current locale
...
__PURE_APPDOMAIN_GLOBAL static locale classic_locale(
    _Noinit);  // "C" locale object, uninitialized

__PURE_APPDOMAIN_GLOBAL locale::_Locimp* locale::_Locimp::_Clocptr =
    0; // pointer to classic_locale
...
__PURE_APPDOMAIN_GLOBAL int locale::id::_Id_cnt =
    0;    // unique id counter for facets
...
__PURE_APPDOMAIN_GLOBAL locale::id codecvt<unsigned short, char, mbstate_t>::id(
    0);
_MRTIMP2 locale::id& __cdecl
codecvt<unsigned short, char, mbstate_t>::_Id_func()
{
...
_MRTIMP2 void __cdecl _Locinfo::_Locinfo_ctor(_Locinfo* pLocinfo,
        const char* locname)
{
