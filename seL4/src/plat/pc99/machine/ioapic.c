...
{
    *(volatile uint32_t*)((uint32_t)(PPTR_IOAPIC_START + ioapic * BIT(
                                         PAGE_BITS)) + reg) = value;
}

static uint32_t ioapic_read(uint32_t ioapic, uint32_t reg)
{
    return *(volatile uint32_t*)((uint32_t)(PPTR_IOAPIC_START + ioapic * BIT(
            PAGE_BITS)) + reg);
}
...
static void single_ioapic_init(uint32_t ioapic, cpu_id_t ioapic_id,
                               cpu_id_t delivery_cpu)
{
    uint32_t id_reg;
    uint32_t i;
    ...
    for (i = 0; i < IOAPIC_IRQ_LINES; i++) {
        ...
        ioapic_write(ioapic, IOAPIC_WINDOW, (ioapic_read(ioapic,
                                             IOAPIC_WINDOW) & MASK(IOREDTBL_HIGH_RESERVED_BITS)) | (delivery_cpu <<
                                                     IOREDTBL_HIGH_RESERVED_BITS));
...
compile_assert(ioapic_id_will_not_overflow,
               CONFIG_MAX_NUM_NODES + CONFIG_MAX_NUM_IOAPIC < 16);
