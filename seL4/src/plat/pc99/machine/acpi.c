...
acpi_table_init(void* entry, enum acpi_type table_type)
{
    ...
    case ACPI_RSDP: {
            ...
            pages_for_table = (rsdp_entry->length + offset_in_page) / MASK(pageBitsForSize(
                                  IA32_4M)) + 1;
            break;
        }
    ...
    case ACPI_RSDT: { // RSDT, MADT, DMAR etc.
            ...
            pages_for_table = (rsdt_entry->header.length + offset_in_page) / MASK(
                                  pageBitsForSize(IA32_4M)) + 1;
            break;
        }
...
)
{
    ...
    *num_ioapic = 0;
    ...
    entries = (acpi_rsdt_mapped->header.length - sizeof(acpi_header_t)) / sizeof(
                  acpi_header_t*);
    ...
    for (count = 0; count < entries; count++) {
        ...
        if (strncmp(acpi_str_apic, acpi_madt_mapped->header.signature, 4) == 0) {
            ...
            while ((char*)acpi_madt_header < (char*)acpi_madt_mapped +
                   acpi_madt_mapped->header.length) {
                ...
                case MADT_IOAPIC:
                    ...
                    if (*num_ioapic == CONFIG_MAX_NUM_IOAPIC) {
                        ...
                        ioapic_paddrs[*num_ioapic] = ((acpi_madt_ioapic_t*)
                                                      acpi_madt_header)->ioapic_addr;
                        (*num_ioapic)++;
                    }
...
)
{
    ...
    *num_passthrough_dev = 0;
    ...
    entries = (acpi_rsdt->header.length - sizeof(acpi_header_t)) / sizeof(
                  acpi_header_t*);
    ...
    for (count = 0; count < entries; count++) {
        ...
        if (strncmp("DMAR", acpi_dmar_mapped->header.signature, 4) == 0) {
            ...
            while ((char*)acpi_dmar_header < (char*)acpi_dmar_mapped +
                   acpi_dmar_mapped->header.length) {
                ...
                case DMAR_RMRR:
                    ...
                    printf("ACPI: RMRR providing region 0x%x-0x%x\n", acpi_dmar_rmrr->reg_base[0],
                           acpi_dmar_rmrr->reg_limit[0]);

                    for (i = 0;
                         i <= (acpi_dmar_header->length - sizeof(acpi_dmar_rmrr_t)) / sizeof(
                             acpi_dmar_devscope_t); i++) {
