...
static vtd_cte_t* lookup_vtd_context_slot(cap_t cap)
{
    ...
    vtd_context_index = (get_pci_dev(pci_request_id) << 3) | get_pci_fun(
                            pci_request_id);
...
static lookupIOPTSlot_ret_t lookupIOPTSlot_helper(vtd_pte_t* iopt,
        word_t translation, word_t levels)
{
    ...
    if (!vtd_pte_ptr_get_write(vtd_pte_slot) || levels == 0) {
        ...
        vtd_next_level_iopt = (vtd_pte_t*)paddr_to_pptr(vtd_pte_ptr_get_addr(
                                  vtd_pte_slot));
        ...
        word_t io_address)
{
    ...
    if (iopt == 0) {
        ...
        return lookupIOPTSlot_helper(iopt, io_address >> PAGE_BITS,
                                     ia32KSnumIOPTLevels - 1);
    }
}
...
)
{
    ...
    paddr = pptr_to_paddr(VTD_PTE_PTR(cap_io_page_table_cap_get_capIOPTBasePtr(
                                          cap)));
...
)
{
    ...
    if (!vtd_cte_ptr_get_present(vtd_context_slot)) {
        ...
        if ((frame_cap_rights == VMReadOnly) &&
            cap_rights_get_capAllowRead(dma_cap_rights_mask)) {
            /* Read Only */
            vtd_pte_ptr_new(
                lu_ret.ioptSlot,
                paddr,  /* Physical Address */
...
void deleteIOPageTable(cap_t io_pt_cap)
{
    ...
    if (cap_io_page_table_cap_get_capIOPTIsMapped(io_pt_cap)) {
        ...
        if (level == 0) {
            ...
            if (pptr_to_paddr(vtd_pte) != cap_io_page_table_cap_get_capIOPTBasePtr(
                    io_pt_cap)) {
                return;
            }
            ...
            if (vtd_pte_ptr_get_addr(lu_ret.ioptSlot) !=
                cap_io_page_table_cap_get_capIOPTBasePtr(io_pt_cap)) {
                return;
            }
