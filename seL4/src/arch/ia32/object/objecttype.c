...
cap_t CONST Arch_updateCapData(bool_t preserve, word_t data, cap_t cap)
{
    ...
    case cap_io_space_cap: {
            ...
            vtd_cte_t* vtd_context_table = (vtd_cte_t*)vtd_rte_get_ctp(
                                               ia32KSvtdRootTable[get_pci_bus(data)]);

            if (!preserve && cap_io_space_cap_get_capPCIDevice(cap) == 0 &&
                vtd_rte_get_present(ia32KSvtdRootTable[get_pci_bus(data)]) &&
                (!vtd_cte_get_present(vtd_context_table[PCIDevice & 0xff]) ||
                 vtd_cte_get_translation_type(vtd_context_table[PCIDevice & 0xff]) != 2) &&
...
)
{
    ...
    case cap_page_table_cap:
    case cap_frame_cap:
        return decodeIA32MMUInvocation(label, length, cptr, slot, cap, extraCaps,
                                       buffer);
    ...
    case cap_io_port_cap:
        return decodeIA32PortInvocation(label, length, cptr, slot, cap, extraCaps,
                                        buffer);
#ifdef CONFIG_IOMMU
