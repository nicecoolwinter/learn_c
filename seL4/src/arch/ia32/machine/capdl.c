...
static void sendPD(unsigned int address)
{
    ...
    for (i = 0; i < PD_READ_SIZE; i++) {
        ...
        if (pde_get_page_size(pde) == pde_pde_4k &&
            (pde_pde_4k_get_pt_base_address(pde) == 0 ||
             !pde_pde_4k_get_present(pde) || !pde_pde_4k_get_super_user(pde))) {
            exists = 0;
        } else if (pde_get_page_size(pde) == pde_pde_4m &&
                   (pde_pde_4m_get_page_base_address(pde) == 0 ||
...
static void sendPT(unsigned int address)
{
    ...
    for (i = 0; i < PT_READ_SIZE; i++) {
        ...
        if (pte_get_page_base_address(pte) != 0 && pte_get_present(pte) &&
            pte_get_super_user(pte)) {
            sendWord(i);
