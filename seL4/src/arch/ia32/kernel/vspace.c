...
)
{
    ...
    while (idx - 1 < (ndks_p_reg.end - ndks_p_reg.start) >> pageBitsForSize(
               IA32_4K)) {
        pte = pte_new(
                  phys,   /* page_base_address    */
                  0,      /* avl                  */
    ...
    while (idx < (PPTR_KDEV& MASK(pageBitsForSize(IA32_4M))) >> pageBitsForSize(
               IA32_4K)) {
        pte = pte_new(
                  0,      /* page_base_address    */
                  0,      /* avl                  */
    ...
    assert(idx == (PPTR_APIC & MASK(pageBitsForSize(IA32_4M))) >> pageBitsForSize(
               IA32_4K));
    pt[idx] = pte;
    idx++;
    ...
    for (i = 0; i < num_ioapic; i++) {
        ...
        assert(idx == ((PPTR_IOAPIC_START + i * BIT(pageBitsForSize(IA32_4K))) & MASK(
                           pageBitsForSize(IA32_4M))) >> pageBitsForSize(IA32_4K));
        pt[idx] = pte;
        idx++;
    ...
    for (i = 0; i < num_drhu; i++) {
        ...
        assert(idx == ((PPTR_DRHU_START + i * BIT(pageBitsForSize(IA32_4K))) & MASK(
                           pageBitsForSize(IA32_4M))) >> pageBitsForSize(IA32_4K));
        pt[idx] = pte;
        idx++;
...
map_temp_boot_page(void* entry, uint32_t pages)
{
    ...
    unsigned int phys_pg_start = (unsigned int)(entry) & ~MASK(pageBitsForSize(
                                     IA32_4M));
    ...
    for (i = 0; i < pages; ++i) {
        unsigned int pg_offset = i << pageBitsForSize(
                                     IA32_4M); // num pages since start * page size
...
static pde_t CONST makeUserPDE(paddr_t paddr, vm_attributes_t vm_attr,
...
static pte_t CONST makeUserPTE(paddr_t paddr, vm_attributes_t vm_attr,
                               vm_rights_t vm_rights)
{
    return pte_new(
               paddr,                                          /* page_base_address    */
        ...
        cte_t* pdCapSlot)
{
    ...
    poolPtr->array[asid & MASK(asidLowBits)] = PDE_PTR(
                cap_page_directory_cap_get_capPDBasePtr(pdCapSlot->cap));
...
)
{
    ...
    if (((pde_ptr_get_page_size(pdSlot) == pde_pde_4k) &&
         pde_pde_4k_ptr_get_present(pdSlot)) ||
        ((pde_ptr_get_page_size(pdSlot) == pde_pde_4m) &&
         pde_pde_4m_ptr_get_present(pdSlot))) {
...
)
{
    ...
    case cap_page_table_cap:
        return decodeIA32PageTableInvocation(label, length, cte, cap, extraCaps,
                                             buffer);
