...
init_irqs(cap_t root_cnode_cap, bool_t mask_irqs)
{
    ...
    write_slot(SLOT_PTR(pptr_of_cap(root_cnode_cap), BI_CAP_IRQ_CTRL),
...
BOOT_CODE cap_t
create_mapped_it_frame_cap(cap_t pd_cap, pptr_t pptr, vptr_t vptr, asid_t asid,
                           bool_t use_large)
{
...
)
{
    ...
    for (i = 0; i < dev_p_regs->count; i++) {
        ...
        for (f = dev_reg.start; f < dev_reg.end;
             f += BIT(pageBitsForSize(frame_size))) {
...
)
{
    ...
    if (!vtd_init(cpu_id, num_drhu, pci_bus_used_bitmap, num_passthrough_dev,
    ...
    write_slot(SLOT_PTR(pptr_of_cap(root_cnode_cap), BI_CAP_IO_SPACE),
               master_iospace_cap());
#endif
