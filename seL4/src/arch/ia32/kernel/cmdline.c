...
static int parse_opt(const char* cmdline, const char* opt, char* value,
                     int bufsize)
{
    int len = -1;
    ...
    while (true) {
        ...
        for (optptr = opt; *optptr && *cmdline && (*cmdline != '=') &&
             !is_space(*cmdline) && (*optptr == *cmdline); optptr++, cmdline++);
...
static int parse_bool(const char* cmdline, const char* opt)
{
    ...
    while (1) {
        ...
        for (optptr = opt; *optptr && *cmdline && !is_space(*cmdline) &&
...
void cmdline_parse(const char* cmdline, cmdline_opt_t* cmdline_opt)
{
    ...
    if (parse_opt(cmdline, "console_port", cmdline_val,
                  MAX_CMDLINE_VAL_LEN) != -1) {
        parse_uint16_array(cmdline_val, cmdline_opt->console_port,
    ...
    if (parse_opt(cmdline, cmdline_str_max_num_nodes, cmdline_val,
                  MAX_CMDLINE_VAL_LEN) != -1) {
    ...
    if (parse_opt(cmdline, cmdline_str_num_sh_frames, cmdline_val,
                  MAX_CMDLINE_VAL_LEN) != -1) {
        i = str_to_int(cmdline_val);
