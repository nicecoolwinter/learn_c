...
typedef struct glks {
    ...
    ui_info_t
    ui_info_list   [CONFIG_MAX_NUM_NODES]; /* info about userland images */
    ...
    paddr_t
    drhu_list[MAX_NUM_DRHU]; /* list of physical addresses of the IOMMUs */
...
load_boot_module(node_id_t node, multiboot_module_t* boot_module,
                 paddr_t load_paddr)
{
...
split_region(unsigned int i, unsigned int n, p_region_t reg)
{
    ...
    if (i < frames_mod) {
        ...
        offset = (frames_mod * (frames_div + 1) + (i - frames_mod) * frames_div) <<
                 PAGE_BITS;
        return (p_region_t) {
            .start = reg.start + offset,
...
try_boot_node(void)
{
    ...
    p_region_t this_avail_p_reg = split_region(node_id, num_nodes,
                                  glks.avail_p_reg);
...
)
{
    ...
    memcpy((void*)BOOT_NODE_PADDR, _boot_cpu_start,
           _boot_cpu_end - _boot_cpu_start);

    /* calculate available physical memory (above 1M) */
    glks.avail_p_reg.start = 0x100000;
    glks.avail_p_reg.end = ROUND_DOWN(glks.avail_p_reg.start +
                                      (mbi->mem_upper << 10), PAGE_BITS);
    ...
    glks.num_nodes = acpi_madt_scan(acpi_rsdt, glks.cpu_list, CONFIG_MAX_NUM_NODES,
                                    &glks.num_ioapic, glks.ioapic_paddr);
    ...
    for (i = mbi->mod_count; i < glks.num_nodes; i++) {
        ...
        load_paddr = load_boot_module(i, mbi->mod_list + mbi->mod_count - 1,
                                      load_paddr);
    ...
    glks.sh_p_reg.end = glks.sh_p_reg.start + (cmdline_opt.num_sh_frames <<
                        PAGE_BITS);

    if (glks.sh_p_reg.end > glks.avail_p_reg.end ||
        glks.sh_p_reg.end < glks.sh_p_reg.start) {
