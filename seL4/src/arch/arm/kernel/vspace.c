...
{
    uint32_t idx = (vaddr & MASK(pageBitsForSize(ARMSection))) >> pageBitsForSize(
                       ARMSmallPage);
...
unmapPage(vm_page_size_t page_size, asid_t asid, vptr_t vptr, void* pptr)
{
    ...
    case ARMSuperSection: {
            ...
            cleanCacheRange_PoU((word_t)&pd[0], LAST_BYTE_PDE(pd,
                                SECTIONS_PER_SUPER_SECTION),
                                addrFromPPtr(&pd[0]));
...
flushPage(vm_page_size_t page_size, pde_t* pd, asid_t asid, word_t vptr)
{
    ...
    if (pde_pde_invalid_get_stored_asid_valid(stored_hw_asid)) {
        ...
        /* Do the TLB flush */
        invalidateTLB_VAASID(base_addr | pde_pde_invalid_get_stored_hw_asid(
                                 stored_hw_asid));
