...
alloc_region(uint32_t size_bits)
{
    ...
    unsigned int reg_index =
        0; /* gcc cannot work out that this will not be used uninitialized */
    ...
    for (i = 0; i < MAX_NUM_FREEMEM_REG; i++) {
        ...
        if (ROUND_UP(ndks_boot.freemem[i].start,
                     size_bits) - ndks_boot.freemem[i].start <
        ...
        if (new_reg.end > new_reg.start &&
            ...
            if (new_reg.start - ndks_boot.freemem[i].start < ndks_boot.freemem[i].end -
                new_reg.end) {
            ...
            if (is_reg_empty(reg) ||
                ...
                (reg_size(new_rem_small) == reg_size(rem_small) &&
                 reg_size(new_rem_large) < reg_size(rem_large))) {
                reg = new_reg;
...
)
{
    ...
    for (f = reg.start; f < reg.end; f += BIT(PAGE_BITS)) {
        if (do_map) {
            frame_cap = create_mapped_it_frame_cap(pd_cap, f, f - BASE_OFFSET - pv_offset,
                                                   IT_ASID, false);
        } else {
...
)
{
    ...
    dc_ret = deriveCap(SLOT_PTR(pptr_of_cap(root_cnode_cap), BI_CAP_IT_IPCBUF),
                       ipcbuf_cap);
...
)
{
    ...
    while (!is_reg_empty(reg)) {
        ...
        if (!provide_untyped_cap(root_cnode_cap, reg.start, size_bits,
                                 first_untyped_slot)) {
            return false;
        }
...
create_untypeds(cap_t root_cnode_cap, region_t boot_mem_reuse_reg)
{
    ...
    if (!create_untypeds_for_region(root_cnode_cap, boot_mem_reuse_reg,
                                    slot_pos_before)) {
        return false;
    }
