...
int  main(int argc, const char* argv[])
{
    ...
    Mat draw(DRAW_W, DRAW_H, CV_8UC3, Scalar(0, 0,
             0)); //建立一个显示直方图的图片，背景为纯黑色
    ...
    while (1) {
        ...
        if (tracking) {
            ...
            if (nFrame_num == 1) {
                ...
                //---------------------------------------------------------------------------------------------------------------------
                double distence = compareHist(staRoiHist, RoiHist,
                                              CV_COMP_INTERSECT); //计算后面选定的与这次选定的相似度，使用INTERSECT，值越大越相似
                printf("\n\t>与第1次选定的图像区域相似度为：%f\n",
                       distence); //数组越大，相似度越大
                ...
                for (int i = 1; i < histsize1; i++) {
                    ...
                    line(draw, Point(DRAW_BIN_W * (i - 1),
                                     DRAW_H - cvRound(rhist.at<float>((i - 1)))), Point(DRAW_BIN_W * (i),
                                             DRAW_H - cvRound(rhist.at<float>(i))), Scalar(255, 0, 0), 2, 8, 0);
                    line(draw, Point(DRAW_BIN_W * (i - 1),
                                     DRAW_H - cvRound(ghist.at<float>((i - 1)))), Point(DRAW_BIN_W * (i),
                                             DRAW_H - cvRound(ghist.at<float>(i))), Scalar(0, 255, 0), 2, 8, 0);
                    line(draw, Point(DRAW_BIN_W * (i - 1),
                                     DRAW_H - cvRound(bhist.at<float>((i - 1)))), Point(DRAW_BIN_W * (i),
                                             DRAW_H - cvRound(bhist.at<float>(i))), Scalar(0, 0, 255), 2, 8, 0);
                }

                imshow("rgb_hist", draw);
                draw = Mat::zeros(DRAW_W, DRAW_H,
                                  CV_8UC3); //每画完一次直方图后都进行一次清0操作
                comp = false;
            }
        ...
        rectangle(frame, preselectROI, Scalar(0, 0, 255), 5,
                  8); //初始的选定目标一直不变
        imshow("camera", frame);
