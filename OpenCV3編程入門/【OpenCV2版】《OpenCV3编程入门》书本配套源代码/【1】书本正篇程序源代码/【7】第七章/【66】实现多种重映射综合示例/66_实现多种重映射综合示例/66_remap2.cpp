...
int main(int argc, char** argv)
{
    ...
    while (1) {
        ...
        update_map(key);
        remap(g_srcImage, g_dstImage, g_map_x, g_map_y, CV_INTER_LINEAR,
        ...
        for (int i = 0; i < g_srcImage.cols; i++) {
            ...
            case '1': // 键盘【1】键按下，进行第一种重映射操作
                if (i > g_srcImage.cols * 0.25 && i < g_srcImage.cols * 0.75 &&
                    ...
                    g_map_x.at<float>(j, i) = static_cast<float>(2 * (i - g_srcImage.cols * 0.25) +
                                              0.5);
                    g_map_y.at<float>(j, i) = static_cast<float>(2 * (j - g_srcImage.rows * 0.25) +
                                              0.5);
                } else {
